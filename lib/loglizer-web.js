...
var LogLizer_web = function(conf_globs){
	...
	this.app.all("/settings", function(req, res) {
		res.render('settings')
	});
	
	this.app.all("/streams", function(req, res) {
		res.render('streams')
	});
	
	this.app.all('/streams/:stream', function(req, res){
		res.render(__dirname + "/../streams/" + req.params.stream + "/web/views/" + req.params.stream);
	});
	
	this.app.all(/\/streams\/([^\/]+)\/(.*)/, function(req, res, next){
